...
@implementation TimePitchAudioIO
...
- (OSStatus)initAVAudio:(NSString *)strFilePath
{
	...
	// Nodeの接続
	[_audioEngine connect:_audioPlayerNode to:_audioUnitTimePitch format:_audioFile.processingFormat];
	[_audioEngine connect:_audioUnitTimePitch to:_audioEngine.mainMixerNode format:_audioFile.processingFormat];
...
{
	_audioUnitTimePitch.rate = fValue;
}

- (void)setPitchParam:(Float32)fValue
{
	_audioUnitTimePitch.pitch = fValue;
}
...
{
	Float32 value = 0.0;
	OSStatus rt = AudioUnitGetParameter(_audioUnitTimePitch.audioUnit,
										parameter,
										kAudioUnitScope_Global,
										0,
										&value);
...
- (void)setValue:(NSInteger)iIndex value:(Float32)value forParameter:(AudioUnitParameterID)parameter min:(Float32)min max:(Float32)max
{
	...
	OSStatus rt = AudioUnitSetParameter(_audioUnitTimePitch.audioUnit,
										parameter,
										kAudioUnitScope_Global,
										0,
										value,
